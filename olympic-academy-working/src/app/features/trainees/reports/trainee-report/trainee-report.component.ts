...
@Component({
  ...
  imports: [
    ...
    MatIconModule, MatTabsModule, SearchableSelectComponent
  ],
  template: `
      ...
      <mat-card>
        ...
        <mat-tab-group>
          ...
          <mat-tab label="قائمة المتدربين">
            <div class="filters">
              <mat-form-field appearance="outline" class="filter-field">
                ...
                <input matInput [(ngModel)]="searchText" (ngModelChange)="filterTrainees()" placeholder="اسم أو رقم هوية">
              ...
              <app-searchable-select
                ...
                (ngModelChange)="genderFilter = $event; filterTrainees()"
                label="الجنس"
              ...
              <app-searchable-select
                ...
                (ngModelChange)="statusFilter = $event; filterTrainees()"
                label="الحالة"
              ...
              <div class="actions">
                <button mat-raised-button color="primary" (click)="loadTrainees()">
            ...
            <div class="summary-cards">
              <div class="summary-card"><h3>إجمالي المتدربين</h3><p>{{ allTrainees.length }}</p></div>
              <div class="summary-card success"><h3>نشط</h3><p>{{ activeCount }}</p></div>
              <div class="summary-card warning"><h3>غير نشط</h3><p>{{ inactiveCount }}</p></div>
            </div>

            <div class="table-container">
              <table class="full-width-table">
                <thead>
                  <tr><th>#</th><th>الاسم</th><th>رقم الهوية</th><th>السنة الدراسية</th><th>الجنس</th><th>الحالة</th><th>عدد الشهادات</th><th>عدد التسجيلات</th></tr>
                </thead>
                <tbody>
                  <tr *ngFor="let t of filteredTrainees; let i=index">
                    <td>{{ i+1 }}</td>
                    <td>{{ t.fullName }}</td>
                    <td>{{ t.nationalId }}</td>
                    <td>{{ t.academicYear || '-' }}</td>
                    <td>{{ t.gender?.title }}</td>
                    <td><span class="badge" [class.active]="t.isActive">{{ t.isActive ? 'نشط' : 'غير نشط' }}</span></td>
                    <td>{{ t.certificates?.length || 0 }}</td>
                    <td>{{ t.enrollmentsCount || 0 }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <mat-tab label="تفاصيل متدرب">
            <div class="filters">
              <app-searchable-select
                [ngModel]="selectedTraineeId"
                (ngModelChange)="selectedTraineeId = $event; loadTraineeDetails()"
                label="اختر المتدرب"
            ...
            <div class="trainee-details" *ngIf="selectedTraineeDetails">
              <div class="details-card">
                <h3>{{ selectedTraineeDetails.fullName }}</h3>
                <p><strong>رقم الهوية:</strong> {{ selectedTraineeDetails.nationalId }}</p>
                <p><strong>السنة الدراسية:</strong> {{ selectedTraineeDetails.academicYear || '-' }}</p>
                <p><strong>الجنس:</strong> {{ selectedTraineeDetails.gender?.title }}</p>
                <p><strong>العنوان:</strong> {{ selectedTraineeDetails.address || '-' }}</p>
                <p><strong>تاريخ الميلاد:</strong> {{ selectedTraineeDetails.birthDate | date }}</p>
                <p><strong>الحالة:</strong> <span class="badge" [class.active]="selectedTraineeDetails.isActive">{{ selectedTraineeDetails.isActive ? 'نشط' : 'غير نشط' }}</span></p>
              </div>

              <div class="details-card">
                <h4>جهات الاتصال</h4>
                <p *ngFor="let c of selectedTraineeDetails.contacts">{{ c.contactType?.title }}: {{ c.contactValue }}</p>
              </div>
              ...
              <div class="details-card">
                ...
                <div class="table-container">
                  <table class="full-width-table">
                    <thead><tr><th>اسم الشهادة</th><th>رقم الشهادة</th><th>الدورة</th><th>تاريخ الإصدار</th><th>الدرجة</th></tr></thead>
                    <tbody>
                      <tr *ngFor="let cert of selectedTraineeDetails.certificates">
              ...
              <div class="details-card">
                <h4>الحالات الصحية</h4>
                <div class="table-container">
                  <table class="full-width-table">
                    <thead><tr><th>العنوان</th><th>الوصف</th><th>العلاج</th><th>ملاحظات</th></tr></thead>
                    <tbody>
                      <tr *ngFor="let h of selectedTraineeDetails.healthConditions">
                        <td>{{ h.title }}</td>
                        <td>{{ h.description }}</td>
                        <td>{{ h.medication }}</td>
                        <td>{{ h.note }}</td>
              ...
              <div class="details-card">
                ...
                <div class="table-container">
                  <table class="full-width-table">
                    <thead><tr><th>الدورة</th><th>المدرب</th><th>تاريخ البدء</th><th>حالة الدفع</th><th>المبلغ</th></tr></thead>
                    <tbody>
                      <tr *ngFor="let e of traineeEnrollments">
                        <td>{{ e.course?.title }}</td>
                        <td>{{ e.trainer?.title }}</td>
                        <td>{{ e.startDate | date }}</td>
                        <td>{{ e.paymentStatus?.title }}</td>
                        <td>{{ e.finalSubscriptionValue | currency:'EGP' }}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
...
export class TraineeReportComponent implements OnInit {
  ...
  loadTraineeOptions() {
    this.traineeOptions = this.allTrainees.map(t => ({ 
      value: t.id, 
      label: `${t.fullName} - ${t.nationalId}` 
    }));
  }
  ...
  filterTrainees() {
    this.filteredTrainees = this.allTrainees.filter(t => {
      const matchSearch = !this.searchText || 
        t.fullName?.includes(this.searchText) || 
        t.nationalId?.includes(this.searchText);
      const matchGender = this.genderFilter === null || t.gender?.id === this.genderFilter;
  ...
  loadTraineeDetails() {
    ...
    this.traineeService.getTraineeById(this.selectedTraineeId).subscribe({
      ...
      error: () => this.notification.showError('حدث خطأ في تحميل تفاصيل المتدرب')
    });
    this.enrollmentService.getAllEnrollmentsByFilter({ traineeId: this.selectedTraineeId }).subscribe({
      ...
      error: () => this.notification.showError('حدث خطأ في تحميل تسجيلات المتدرب')
    });
  }
  ...
  exportToExcel() {
    ...
    const data = this.filteredTrainees.map((t, i) => ({ 
      '#': i + 1, 
      'الاسم': t.fullName, 
      ...
      'عدد الشهادات': t.certificates?.length || 0 
    }));
